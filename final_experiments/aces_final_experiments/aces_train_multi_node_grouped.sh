...
#SBATCH --output=aces_grouped_slurm_logs/grouped-ulysses-%j.output
#SBATCH --error=aces_grouped_slurm_logs/grouped-ulysses-%j.err
...
CMD="train_gpt_grouped.py --seq_length ${SEQUENCE_LENGTH} --seq_parallel_size ${SEQ_PARALLEL_SIZE}"

export HF_TOKEN=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
export TRANSFORMERS_OFFLINE=1
export HF_DATASETS_OFFLINE=1
export HF_HUB_OFFLINE=1
mkdir -p aces_group_exp
...
srun --export=ALL -l bash -c "${LAUNCHER} ${CMD}" > "aces_group_exp/group_run_${SEQUENCE_LENGTH}.log" 2>&1
