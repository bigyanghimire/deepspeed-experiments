...
def parse_summary_line(line):
    ...
    pattern = r"SUMMARY: config=(\S+) batch_size=(\d+) params=(\d+) peak_mem_bytes=(\d+) alloc_mem_bytes=(\d+) avg_step_time=(\S+)"
    match = re.search(pattern, line)
    if match:
        return {
            "config": match.group(1),
            "params": int(match.group(3)),
            "peak_mem_bytes": int(match.group(4)),
            "alloc_mem_bytes": int(match.group(5)),
            "avg_step_time": float(match.group(6)),
        }
    return None
