...
for iter, batch in enumerate(dl):
    ...
    if dist.get_rank() == 0:
        print(f"Batch shapes in rank 0:")
        print(f"  input_ids in rank {dist.get_rank()}: {batch['input_ids'].shape}")  # Should be [batch_size, seq_len/sp_size]
        print(f"  input_ids value rank in iter {iter} {dist.get_rank()}: {batch['input_ids']}")  # Should be [batch_size, seq_len/sp_size]
        print(f"  position_ids in rank {dist.get_rank()}: {batch['position_ids'].shape}")
        print(f"  labels in rank {dist.get_rank()}: {batch.get('labels', 'N/A')}")
    
    if dist.get_rank() == 1:
        print(f"Batch shapes in rank 1:")
        print(f"  input_ids in rank {dist.get_rank()}: {batch['input_ids'].shape}")  # Should be [batch_size, seq_len/sp_size]
        print(f"  input_ids value rank in iter {iter} {dist.get_rank()}: {batch['input_ids']}")  # Should be [batch_size, seq_len/sp_size]
        print(f"  position_ids in rank {dist.get_rank()}: {batch['position_ids'].shape}")
        print(f"  labels in rank {dist.get_rank()}: {batch.get('labels', 'N/A')}")
