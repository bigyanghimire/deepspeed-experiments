...
## This has Pytorch profiler integration 
...
def main():
    ...
    with torch.profiler.profile(
        ...
        for step in range(args.num_steps):
            ...
            dataloading_time = time.time() - dataloading_start_time 
            # Forward pass with optional autocast
            forwardpass_start_time = time.time()
            with record_function("## forward ##"):
                if use_autocast:
                    ...
                    logits = model_engine(input_ids)
                    loss = loss_fn(logits.view(-1, actual_vocab_size), labels.view(-1))
            forwardpass_time = time.time() - forwardpass_start_time
